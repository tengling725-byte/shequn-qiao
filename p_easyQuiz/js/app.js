...
const QuizApp = {
  ...
  renderSetup() {
    ...
    return '<div class="page setup">' +
      ...
      '<div class="setting-group">' +
        '<label>模式</label>' +
        '<div class="btn-group">' +
          '<button class="btn' + (this.state.mode === 'exam' ? ' active' : '') + '" onclick="QuizApp.setMode(\'exam\')">���试模式</button>' +
  ...
  renderQuiz() {
    ...
    let optionsHtml = q.options.map(opt => {
      ...
      return '<button type="button" class="' + classes + '" onclick="window.QuizApp.selectAnswer(' + opt.id + ')"><span>' + opt.text + '</span></button>';
    }).join('');
  ...
  renderResult() {
    ...
    let analysisHtml = shuffledQuestions.map((q, i) => {
      ...
      return '<div class="analysis-item ' + (isCorrect ? 'correct' : 'wrong') + '">' +
        ...
        '<div class="q-answer">' +
          '你的答案: ' + userAnswerText +
          (!isCorrect ? '<br>正确答案: ' + correctAnswerText : '') +
        '</div>' +
        (q.explanation ? '<div class="q-explanation">解析: ' + q.explanation + '</div>' : '') +
      '</div>';
    }).join('');
