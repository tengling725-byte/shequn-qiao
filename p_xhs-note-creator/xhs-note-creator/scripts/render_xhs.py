...
def convert_markdown_to_html(md_content: str, base_dir: str = None) -> str:
    """将 Markdown 转换为 HTML"""
    # 处理 tags（以 # 开头的标签）
    ...
    if base_dir:
        ...
        def replace_img_path(match):
            ...
            # 如果不是绝对路径或 data URI，则转换为绝对路径
            if not img_src.startswith(("http://", "https://", "data:", "/")):
                abs_path = os.path.abspath(os.path.join(base_dir, img_src))
                img_tag = img_tag.replace(img_src, f"file://{abs_path}")
            return img_tag

        html = re.sub(r'<img[^>]+src=["\']([^"\']+)["\'][^>]*>', replace_img_path, html)
...
def generate_card_html(
    ...
    html_content = convert_markdown_to_html(content, base_dir)
...
async def render_markdown_to_cards(
    ...
    for i, content in enumerate(card_contents, 1):
        ...
        card_html = generate_card_html(
            content, theme, i, total_cards, width, height, mode, base_dir
        )
