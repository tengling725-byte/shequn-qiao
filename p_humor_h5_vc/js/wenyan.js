...
var wenyan = {
  ...
  showItem: function() {
    var item = wenyanData[this.currentIndex];
    if (!item) return;
    
    document.getElementById('wenyan-title').textContent = item.title;
    document.getElementById('wenyan-original').textContent = item.original;
    document.getElementById('wenyan-note').textContent = item.note;
    document.getElementById('wenyan-note').style.display = 'none';
    document.getElementById('wenyan-tap').textContent = '点击显示注释';
    document.getElementById('wenyan-index').textContent = (this.currentIndex + 1) + '/' + wenyanData.length;
  },
  
  toggleNote: function() {
    var noteEl = document.getElementById('wenyan-note');
    var tapEl = document.getElementById('wenyan-tap');
    if (noteEl.style.display === 'none') {
      noteEl.style.display = 'block';
      tapEl.textContent = '点击隐藏注释';
    } else {
      noteEl.style.display = 'none';
      tapEl.textContent = '点击显示注释';
    }
  },
  
  prev: function() {
    this.currentIndex--;
    if (this.currentIndex < 0) this.currentIndex = wenyanData.length - 1;
    this.showItem();
  },
  
  next: function() {
    this.currentIndex++;
    if (this.currentIndex >= wenyanData.length) this.currentIndex = 0;
    this.showItem();
  }
};
...
var wenyanData = [
  ...
  { title: "子产论政宽猛", original: "郑子产有疾，谓子太叔曰：'我死，子必为政。唯有德者能以宽服民，其次莫如猛。夫火烈，民望而畏之，故鲜(少)死焉；水懦弱，民狎(轻慢)而玩之，则多死焉。故宽难。'", note: "子产认为'宽政'难以实施，'猛政'虽残酷但有效。体现法家'以刑止刑'思想。" },
