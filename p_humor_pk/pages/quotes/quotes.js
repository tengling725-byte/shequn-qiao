Page({
  data: {
    quotes: [
      ...
      { text: "君处北海，寡人处南海，唯是风马牛不相及也。", author: "《左传》" },
  ...
  onLoad() {
    const favs = wx.getStorageSync('quotesFavs') || [];
    this.setData({ favorites: favs });
  },

  prevQuote() {
    let index = this.data.currentIndex - 1;
    if (index < 0) index = this.data.quotes.length - 1;
    this.setData({ currentIndex: index });
  },

  nextQuote() {
    let index = (this.data.currentIndex + 1) % this.data.quotes.length;
    this.setData({ currentIndex: index });
  },
  ...
  toggleFavorite() {
    ...
    wx.setStorageSync('quotesFavs', favs);
    this.setData({ favorites: favs });
    wx.showToast({ title: favs.includes(index) ? '已收藏' : '已取消', icon: 'none' });
  },
  ...
  touchEnd(e) {
    ...
    const diff = this.data.startX - endX;
    if (Math.abs(diff) > 50) {
